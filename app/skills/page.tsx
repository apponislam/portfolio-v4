    ...
    "Next.js": {
        ...
        icon: (
            <svg className="w-8 h-8 text-black fill-current transition-transform group-hover:scale-105" viewBox="0 0 24 24">
                <path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zm6.2 14.5l-6.8-8.8H9.6v6.4h1.1V10h.1l5.8 7.6c.5-.5.9-1.1 1.6-1.1zM14.4 7.6h1.2v6.4h-1.2V7.6z"/>
            </svg>
        ),
    },
    JavaScript: {
        ...
        icon: (
            <svg className="w-8 h-8 text-[#F7DF1E] fill-current transition-transform group-hover:scale-105" viewBox="0 0 24 24">
                <path d="M24 0H0v24h24V0z" fill="none"/>
                <path d="M24 0H0v24h24V0zm-3.75 18.25c-.25.82-.95 1.34-1.85 1.34-1.15 0-1.82-.75-1.82-1.95v-1.1h1.3v1.1c0 .52.32.8.72.8.42 0 .62-.25.75-.63l1.1.44-.2 1zm-5.75.65c-.3.72-1.1 1.1-1.92 1.1-.9 0-1.64-.52-1.64-1.54 0-1.15.82-1.6 2.3-1.74v-.12c0-.36-.26-.64-.84-.64-.54 0-.82.24-1 .63l-1.05-.62c.42-.92 1.25-1.34 2.15-1.34 1.46 0 2 .8 2 2.12v3.1H14.5v-1z"/>
            </svg>
        ),
    },
    TypeScript: {
        ...
        icon: (
            <svg className="w-8 h-8 text-[#3178C6] fill-current transition-transform group-hover:scale-110" viewBox="0 0 24 24">
    ...
    "Animation Libraries": {
        ...
        icon: (
            <svg className="w-8 h-8 transition-transform group-hover:scale-105" viewBox="0 0 24 24" fill="currentColor">
                <defs>
                    <linearGradient id="framer-grad-2" x1="0%" y1="0%" x2="100%" y2="100%">
                        <stop offset="0%" stopColor="#FF0080" />
                        <stop offset="100%" stopColor="#7928CA" />
                    </linearGradient>
                </defs>
                <path d="M0 0h24L12 12h12v12H0l12-12H0V0z" fill="url(#framer-grad-2)"/>
            </svg>
        ),
    },
    "Shadcn UI": {
                                ...
                                {categorySkills.map((skill) => {
                                    ...
                                    return (
                                        <div key={skill.name} className={`group relative bg-white border border-neutral-300/40 rounded-[24px] p-4 sm:p-5 shadow-sm hover:shadow-[0_15px_35px_rgba(0,0,0,0.04)] hover:-translate-y-1.5 hover:scale-[1.02] transition-all duration-500 flex flex-col items-center justify-center text-center cursor-pointer aspect-square ${style.borderColor} ${style.glow}`}>
                                            ...
                                            {/* Skill Name */}
                                            <span className="font-extrabold text-neutral-800 text-xs sm:text-sm tracking-tight transition-colors duration-500 group-hover:text-black mb-1">
                                                {skill.name}
                                            </span>
                                            
                                            {/* Description */}
                                            {skill.description && (
                                                <p className="text-neutral-600 text-[10px] sm:text-xs font-medium leading-relaxed mt-1 transition-colors duration-500 group-hover:text-neutral-800 max-w-[155px] line-clamp-2">
                                                    {skill.description}
                                                </p>
                                            )}
                                        </div>
